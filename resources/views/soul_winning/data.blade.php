...
@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title">{{trans_choice('general.soul_winning',2)}}</h3>

            <div class="box-tools pull-right mb-2">
                @if(Sentinel::hasAccess('members.create'))
                    <a href="{{ url('soul_winning/create') }}"
                       class="btn btn-info btn-sm">{{trans_choice('general.add',1)}} {{trans_choice('general.soul_winning',1)}}</a>
                @endif
            </div>
        </div>
        <div class="box-body ">
...
@section('footer-scripts')
    ...
    <script>
        $('#data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{!! url('soul_winning/get_soul_winnings?') !!}',
            columns: [
                { data: 'id', name: 'id' },
