...
@section('content')
    <div class="box box-primary">
        ...
        <div class="mb-3 text-black-500 p-3">
            <form  method="POST" action="{{ route('cell.update', $cell->id) }}">
                @method('PATCH')
                @csrf
                ...
                <div class="mt-2">
                    <input id="name" class="form-control" ype="text" name="name"
                        value="{{ old('name') ?? $cell->name }}" placeholder="The cell name" required />
        ...
        "cell_id" => null --}}
                    ...
                    <select name="leader" class="form-control" required>
                        ...
                        @foreach ($members as $key => $member)
                            <option value="{{ $member->id }}" @if ($member->id == $cell->leader) selected @endif>
                ...
                <div class="box-footer">
                    <button type="submit" class="btn btn-primary pull-right" id="add_cell">
                        Update
                    </button>
                </div>
