...
@endsection
@section('title')
    {{trans_choice('general.pledge',1)}}  {{trans_choice('general.payment',2)}}
@endsection
@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title">{{trans_choice('general.pledge',1)}}  {{trans_choice('general.payment',2)}} </h3>

            <div class="box-tools pull-right mb-2">
                @if(Sentinel::hasAccess('pledges.update'))
                    <a href="{{ url('pledge/'.Request::segment(2).'/payment/create') }}"
                       class="btn btn-info btn-sm">{{trans_choice('general.add',1)}} {{trans_choice('general.pledge',1)}}  {{trans_choice('general.payment',1)}} </a>
                @endif
            </div>
        </div>
        <div class="box-body">
...
@section('footer-scripts')
    ...
    <script>
        $('#data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{!! url('pledge/get_pledge_payments?pledge_id='.Request::segment(2)) !!}',
            columns: [
                { data: 'id', name: 'id' },
