...
Route::get('cell/get_cell', 'CellController@get_cell');
...
Route::get('mydashboard', [
    ...
    function () {
        ...
        for ($i = 1; $i < 14; $i++) {
            ...
            $contributions = \App\Models\Contribution::where('year', $d[0])->where(
                'month',
                $d[1]
            )->sum('amount');
            $pledges = \App\Models\PledgePayment::where('year', $d[0])->where('month', $d[1])->sum('amount');
            $other_income = \App\Models\OtherIncome::where('year', $d[0])->where(
                'month',
                $d[1]
            )->sum('amount');
            $events = \App\Models\EventPayment::where('year', $d[0])->where(
                'month',
                $d[1]
            )->sum('amount');
            $ext = ' ' . $d[0];
        ...
        return view('mydashboard', compact('monthly_overview_data', 'events'));
    }
]);
...
Route::group(['prefix' => 'organization'], function () {
    ...
    Route::delete('{id}/delete', 'TagController@delete');
